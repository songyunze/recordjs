...
const replay = {
    replayMouseEvt(item:Record,evt:string): void {
        const args :any = {
            'view': window,
            'bubbles': true,
            'cancelable': true
        ...
        let event = new MouseEvent(evt, args);
    ...
    replayPushState(item:Record):void{
    ...
    replayReplaceState(item:Record):void{
        // pushState 延时触发，因为一般点击事件会触发本事件。
        setTimeout(() => {
            const path = item.state.path;
            var url = /^http/.test(path) ? path : (location.protocol + '//' + location.host + path);
    ...
    replaytouch(item:Record,type:string):void{
        console.log('touch')
